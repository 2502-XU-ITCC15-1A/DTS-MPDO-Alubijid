...
describe('formatStatusChangeTitle', () => {
  it('TC-DASH-04a: produces from?to string when oldStatus given', () => {
    expect(formatStatusChangeTitle('Pending', 'Processing')).toBe(
      'Status changed from Pending to Processing',
    );
  });

  it('TC-DASH-04b: produces to-only string when oldStatus is empty', () => {
    expect(formatStatusChangeTitle('', 'Processing')).toBe(
      'Status changed to Processing',
    );
  });

  it('TC-DASH-04c: works for approval transition', () => {
    expect(formatStatusChangeTitle('Sent for approval', 'Completed')).toBe(
      'Status changed from Sent for approval to Completed',
    );
  });
});
...
describe('getDocumentIdFromQrText', () => {
  it('TC-DASH-05a: extracts DTN from ?doc= URL param', () => {
    expect(
      getDocumentIdFromQrText('https://app.example.com/dashboard?doc=DTN-2026-0001'),
    ).toBe('DTN-2026-0001');
  });

  it('TC-DASH-05b: extracts DTN from DTN: prefix format', () => {
    expect(getDocumentIdFromQrText('DTN:DTN-2026-0001|extra')).toBe('DTN-2026-0001');
  });

  it('TC-DASH-05c: extracts bare DTN-YYYY-NNNN pattern', () => {
    expect(getDocumentIdFromQrText('Reference: DTN-2026-0042')).toBe('DTN-2026-0042');
  });
...
describe('parseStoredList', () => {
  ...
  it('TC-DASH-06a: returns parsed array from valid localStorage entry', () => {
    localStorage.setItem('customDocumentTypes', JSON.stringify(['Memorandum', 'Letter']));
    expect(parseStoredList('customDocumentTypes')).toEqual(['Memorandum', 'Letter']);
  });
...
describe('filteredDocuments search and status logic', () => {
  ...
  it('TC-DASH-09b: DTN search matches by ID', () => {
    const docs = [
      { id: 'DTN-2026-0001', title: 'Doc A' },
      { id: 'DTN-2026-0002', title: 'Doc B' },
    ];
